...
## 7. Sectors Mapping
Sectors OHLCV (ID: `/v2/indonesia/transaction/daily`, SG: `/v2/singapore/transaction/daily`) → DataFrame `{open,high,low,close,volume?,amount?}` + `x_timestamp/y_timestamp` dari `date`; fundamentals → `{roe, margin, leverage, pe, pb, sector, market}`. Missing `volume/amount` → 0.0; missing rasio → sector median fallback 0.0 per market + `insufficient_data:true`. Cache key `market:sector:ticker:date`.
