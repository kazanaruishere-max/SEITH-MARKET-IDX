...
3. **Cadence & Drift Watch — Long-Term Production**
   ...
   - Tiap fase: `seith-phase-gate` (7 reviewers parallel: `code+data+quant+security+design+doc+pm`) — not just `rust∥security` — veto if one FAIL
   ...
   - Pra-freeze MANDATORY: `seith-security-reviewer` + `architect` sign-off + `scripts/freeze-check.sh` + video 1m/3m checklist + `30 Sep 23:59 WIB` — `30% Technical depth` lives here
