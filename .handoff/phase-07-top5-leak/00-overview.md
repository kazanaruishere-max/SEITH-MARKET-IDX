...
- Z5 `research/pyproject.toml` uv isolated `jupyterlab + ipykernel + plotly==5.* + pandas (+ polars opsional)` pinned · `research/money-leak-backtest.ipynb` 7 cells `plotly` `actual zinc vs forecast amber dashed + ±2σ red 10%` + `StackedBar 30/20/30/20` + `hist |Z|>2` + load `fixtures/bbca-400/sector-median/illiquid/kronos-pred-20.json` 0 credit + placeholder `20 ticker Excel venue read_excel` + `research/.python-version 3.11`
...
|---|---|---|---|
| 01 | `01-research-pyproject.md` | `research/pyproject.toml` isolated uv `plotly==5.*` | — |
| 02 | `02-jupyter-backtest.md` | `research/money-leak-backtest.ipynb` 7 cells Plotly `seith-kronos` | 01 |
...
1. `research/pyproject.toml` uv isolated `jupyterlab + ipykernel + plotly==5.* + pandas` pinned — `uv --project research sync` 0 + `uv --project research run jupyter --version` + `plotly==5.*` pinned + `.python-version 3.11`
...
8. Accountability `✅/⚠️/🔻/♻️` per task + `verification-loop` paste nyata no fabrikasi (placeholder until implement, filled after nbconvert/pnpm)
...
|---|---|---|---|---|
| Lead Otak T0 | opencode sini | `seith-market-intelligence` + `verification-loop` + `seith-phase-gate` | — | Understand→Plan→Document + verify 03 |
| Founder | User | — | — | approve 20 ticker + Top5 dulu |
| PM Autonomous | `seith-pm` | `git-worktree-manager`+gate `fmt/clippy/test` + `senior-pm` + `seith-phase-gate` | `seith-pm` | orkestrasi `handoff/07-top5-leak` + veto if `apps/kronos-sidecar` mixed |
| Arsitek | `architect` | `senior-architect` + `seith-phase-gate` | `architect` | **SEBELUM coding** — audit Z5 vs Z2 isolated |
| Eksekutor T1 | sub-agent | `seith-market-intelligence`+`seith-kronos`+`tdd-workflow`+`verification-loop`+`git-worktree-manager`+`no-ai-slop`+`design-taste-frontend`+`seith-phase-gate` | `explore` | 01-02 Z5 research + 03 Z2 web |
...
| Gate Fase | `seith-phase-gate` | `seith-phase-gate`+`verification-loop` | — | penutupan fase dual-review |
| Doc | `doc-updater` | `remember`+`handoff`+`no-ai-slop` | `doc-updater` | sinkron docs |
...
- Tiap session wajib `skill://seith-market-intelligence` + `skill://seith-kronos` (02) + ritual 3Q `docs/notes/00-readme.md` + `no-ai-slop` Tier-1
...
uv --project research sync → 0 (plotly==5.* pinned)
...
## Risks & Mitigasi
- `jupyter/plotly masuk apps/kronos-sidecar` → PM veto + `grep plotly apps/kronos-sidecar/pyproject.toml → 0` + isolated `research/pyproject.toml` `.python-version 3.11`
...
## Next Session Prompt
`skill://seith-market-intelligence` + `skill://seith-kronos` + branch `handoff/07-top5-leak` + task `01-research-pyproject.md` + ritual 3Q
