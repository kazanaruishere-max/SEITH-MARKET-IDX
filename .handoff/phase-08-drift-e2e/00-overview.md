...
| 03 | `03-e2e-manual.md` | 4 proses + ranking + dossier PDF + validation-report | 01+02 | T1+T2 joint |

Dependensi: `H7 76d7208 → 01∥02 file-disjoint → 03 E2E → PR34 H6 Freeze → PR35 H7b → Submit 30 Sep`. Cross-zona dilarang §3c. `9router PID 22484 NEVER kill`.
...
## Branch & Worktree
- Parent: `handoff/08-drift-e2e` dari `main 76d7208` — `git worktree add ../seith-wt/handoff-08 -b handoff/08-drift-e2e`
- T1: `handoff/08-drift-e2e/t1-rust-client` dari parent — `../seith-wt/handoff-08-t1` — Z1 only
- T2: `handoff/08-drift-e2e/t2-combo-wire` dari parent — `../seith-wt/handoff-08-t2` — Z1 config + Z2 analysis + Z5 env only
- File-disjoint: T1 `crates/sectors-client/* + seith-core/market.rs` vs T2 `config.rs + analysis.rs + apps/analysis/* + .env.example` — no conflict
- Merge: T1→parent PR + T2→parent PR → parent→main squash — `/.wt/` gitignore
