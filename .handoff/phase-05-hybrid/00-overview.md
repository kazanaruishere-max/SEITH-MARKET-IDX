...
## Context
- SSOT: `AGENTS.md §3c Seven Zones §8b Branch §8c Ownership` + `docs/spec.md §2[8] §6 §7b` + `docs/api-spec.md §1-10 (envelope + market + 6 endpoints)` + `docs/tdd-plan.md §3 paths 6-7 §4 CLI contract §7 fixtures` + `docs/adr/0001-0005` + `skill://seith-market-intelligence` + `skill://no-ai-slop` Tier-1 + `skill://verification-loop` + `skill://design-taste-frontend` (Z2) + `skill://git-worktree-manager`
...
- Z1 `crates/seith-api/src/{handlers.rs, envelope.rs, repository.rs, lib.rs}` Axum `GET /health /api/v1/health /api/v1/ranking /api/v1/tickers/:ticker/score /api/v1/tickers/:ticker/dossier?format=json|pdf /api/v1/anomalies POST /api/v1/scan` envelope `{success,data,error,pagination}` header `x-schema-version`
...
- Z4 `tests/fixtures/ranking-snapshot.json` CLI vs REST contract — assigned to `04-verify-e2e.md 04f`
...
8. Docs sinkron `spec §2[8] + api-spec §3 + tdd-plan §3/7 + adr/0005-hybrid-delivery.md` + README 7 Zones
...
| Eksekutor T1/T2 | sub-agent | `seith-market-intelligence`+`tdd-workflow`+`verification-loop`+`git-worktree-manager` | `explore` | T1 API+Web ∥ T2 CLI+Dossier (file beda) |
| Desainer Test | `tdd-guide` | `tdd-guide`+`tdd-workflow` | `tdd-guide` | matrix market/envelope/paginate + `deny_unknown_fields` |
| Reviewer Rust | `rust-reviewer` | `code-reviewer` | `code-reviewer` | handlers + cli — `no-ai-slop` check |
| Reviewer Security | `security-reviewer` | `security-review` | `security-reviewer` | market validation + secret env-only + `no-ai-slop` |
| Refactor WAJIB | `refactor-cleaner` | `coding-standards` | `refactor-cleaner` | pasca tiap task — `design-taste-frontend` untuk Z2 |
| Doc | `doc-updater` | `remember`+`handoff` | `doc-updater` | sinkron docs tiap merge — `adr/0005` |
