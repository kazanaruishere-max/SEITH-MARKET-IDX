...
## Scope In / Out
In: Z2 `apps/web lib/api.ts` zod schemas `sort/order/minZ/degraded` + `app/page.tsx hero TopLeaks Top5 + app/ranking + app/dossier/[ticker] + app/backtest` route + `components/TopLeaks.tsx + BacktestChart.tsx + MetricsTable.tsx + ScoreBadge stacked 30/20/30/20 + DossierPDF.tsx 2-page 9-section ID` + `next.config.js rewrites /api/v1 → 8181` + `NEXT_PUBLIC_API_BASE` — Bloomberg `#0B0E14/#11151F` + `JetBrains Mono` — Z6 handoff `00-03` + dossier `lang=id` `peer QV distance+cap±50%` `visual PDF Line±2σ`
...
|---|---|---|---|
| 01 | `01-api-contract.md` | `GET /api/v1/ranking + /anomalies?sort=anomaly&minZ=2.0&pageSize=5 Top5 + /tickers/:ticker/dossier?format=json\|pdf&lang=id + /backtest (static research/backtest-100.json 100 real)` → envelope `success/data/error/pagination + x-schema-version + disclaimer` + `lib/api.ts` zod `sort/order/minZ/degraded/insufficientData/lang/peerComparison[5]` + `next.config.js` proxy + `NEXT_PUBLIC_API_BASE` | — |
| 02 | `02-visual-spec.md` | `TopLeaks.tsx Top5 + BacktestChart.tsx Line+Area ±2σ + MetricsTable + ScoreBadge stacked 30/20/30/20 + DossierPDF.tsx 2-page 9-section ID (Page1 Cover/Executive/Mispricing/Valuation/Peer/Anomali + Page2 Katalis/Metodologi/Annex) + peer rule QV distance+cap±50% + visual Line 400→20 vector` | 01 |
| 03 | `03-journey-integration.md` | Journey `app/page.tsx hero TopLeaks → app/ranking (60s sortable) → app/dossier/[ticker] 2-page ID deep dive peer5 QV+cap + chartPoints Line±2σ + research tabs ID → PDF @react-pdf/renderer 9-section export` + `app/backtest equity vs IHSG` route + `pnpm lint 0 typecheck 0` 40% usability | 02 |
...
1. `00-overview.md` + 01-03 handoff lengkap: Surgical table per task (1 bag = 1 fn <50), Acceptance terukur (`Top5 sort=anomaly pageSize=5 minZ2.0`, `recharts Line + Area ±2σ`, `ScoreBadge stacked 4 segmen`, `DossierPDF 2-page 9-section ID`, `peer QV+cap±50%`, `disclaimer tiap view` `lang=id`), Verification block `cargo fmt 0 clippy --all-targets 0 test 145 + uv 17 + pnpm lint 0 typecheck 0 test + gitleaks 0` + Accountability `✅/⚠️/🔻/♻️` per task + `♻️ Refactor:` placeholder
2. `lib/api.ts` contract documented: `fetchRanking({market,sector,sort,order,page,pageSize,minZ})` + `fetchAnomalies + fetchBacktest + fetchDossier(ticker,{market,format,lang})` zod schemas `RankingQuery + AnomalyQuery + BacktestQuery + DossierQuery(lang,peerComparison[5])` + envelope `success/data/error/pagination + x-schema-version + disclaimer` — `dossier ?lang=id` default (EN future toggle)
3. `next.config.js` rewrites documented: `async rewrites() { return [{ source: '/api/v1/:path*', destination: 'http://localhost:8181/api/v1/:path*' }] }` + `NEXT_PUBLIC_API_BASE` env handling — `NEXT_PUBLIC_API_BASE` only, `SECTORS_API_KEY` server-only
4. Visual spec covers `TopLeaks` gap (`app/page.tsx` hero), `BacktestChart` gap (`recharts 0 import → Line/Area ±2σ`), `MetricsTable` gap (`Sharpe/maxDD/winRate`), `ScoreBadge` stacked 30/20/30/20, `DossierPDF` 2-page 9-section ID (Page1 Cover/Executive/Mispricing/Valuation/Peer QV+cap/Anomali + Page2 Katalis/Metodologi/Annex) + `peer rule QV distance+cap±50% + |Z| tie-break` + `visual Line 400→20 + Area ±2σ vector`
