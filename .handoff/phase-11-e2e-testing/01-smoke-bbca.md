...
## Scope In / Out
In: `curl Sectors BBCA + 9router PONG + Kronos predict_batch 400→20 T1.0 top_p0.9 vol ±2σ` + `cargo fmt/clippy/test 150` + `seith-cli ranking/scan` envelope + `GET /api/v1/tickers/BBCA/dossier?format=json&lang=id peer[5] + ?format=pdf&lang=id %PDF 2 pages` + `pnpm lint/typecheck` + `gitleaks 0 check`
...
- `fn<50` N/A docs+test-only, `file 200-400` observe only, `cargo fmt --check 0 + clippy --all-targets 0 + cargo test 150 + pnpm lint 0 typecheck 0 + uv workdir 17 passed + gitleaks 0 + grep SECTORS_API_KEY apps/web →0 + grep plotly kronos-sidecar →0` — no drift
...
- ✅ Terverifikasi: `cargo fmt 0 + clippy --all-targets 0 + test 150 + uv workdir 17 + pnpm lint0 typecheck0` no drift, `Sectors BBCA probe + 9router PONG + Kronos 400→20 + dossier BBCA json/pdf 2 pages` — output nyata (paste after run)
