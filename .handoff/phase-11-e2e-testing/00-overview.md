...
| 03 | `03-sync-pm-gate.md` | Sync handoff/11 → main + gates fmt0 clippy0 test150 pnpm0 uv17 + seith-pm dual-review PASS veto N → lock H10-impl | 02 |
...
1. `00-overview.md` + 01-03 handoff lengkap: Surgical table per task (1 bag = 1 fn <50), Acceptance terukur (`BBCA rank + |Z| + peer5 reason`, `5 TopLeak + equity12`, `PM gate 8 poin`), Verification block `cargo fmt0 clippy0 test150 + pnpm lint0 typecheck0 + uv17 + gitleaks0 + curl live` + Accountability `✅/⚠️/🔻/♻️` per task + `♻️ Refactor:` placeholder
...
8. Verification: `cargo fmt --check →0 / cargo clippy --all-targets -- -D warnings →0 / cargo test →150 passed (20+7+20+16+87) / uv --project apps/analysis run pytest -q →17 passed / pnpm --dir apps/web lint→0 typecheck→0 / gitleaks detect →0 / sqlite3 observe WAL / curl live BBCA+ PONG + dossier PDF` — paste output nyata no fabrikasi
...
cargo test → 150 passed (20 sectors-client +7 seith-api lib +20 seith-api integration +16 seith-cli +87 seith-core)
