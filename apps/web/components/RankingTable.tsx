        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-zinc-800/60">
            {sorted.map((r) => {
              ...
              const title = `${r.ticker} — ${p?.name ?? r.ticker} — ${p?.desc ?? r.sector ?? ""} — sumber: ${p?.idxUrl ?? "idx.co.id"} — klik → dossier`;
              return (
