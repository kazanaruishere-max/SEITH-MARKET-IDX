...
export type RankingItem = { ticker: string; mispricingScore: number; anomalyZ?: number; sector?: string; market?: string; anomalyFlag?: boolean; rank?: number };
    ...
    <div className="overflow-hidden rounded-xl border border-zinc-800 bg-[#11151F]">
      <table className="w-full text-sm">
        <thead className="bg-[#1A1F2E] text-left text-xs uppercase tracking-wide text-zinc-400">
          <tr><th className="px-3 py-2">Ticker</th><th className="px-3 py-2">Market</th><th className="px-3 py-2">Score</th><th className="px-3 py-2">|Z|</th><th className="px-3 py-2">Rank</th></tr>
        </thead>
        <tbody className="divide-y divide-zinc-800">
          {sorted.map((r) => (
            <tr key={r.ticker} className="hover:bg-zinc-900/40">
              <td className="px-3 py-2 font-mono"><a className="hover:underline" href={`/dossier/${r.ticker}?market=${r.market ?? "id"}`}>{r.ticker}</a></td>
              ...
              <td className="px-3 py-2 text-zinc-400">{(r.anomalyZ ?? 0).toFixed(1)}</td>
              <td className="px-3 py-2 text-zinc-400">{r.rank ?? "-"}</td>
            </tr>
          ))}
          {sorted.length === 0 ? <tr><td colSpan={5} className="px-3 py-8 text-center text-zinc-500">No data — consume /api/v1/ranking</td></tr> : null}
        </tbody>
      </table>
