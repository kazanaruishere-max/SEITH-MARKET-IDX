...
export type RankingItem = { ticker: string; mispricingScore: number; anomalyZ?: number; sector?: string; market?: string; anomalyFlag?: boolean; rank?: number; close?: number; companyProfile?: { name: string; description: string; sourceUrl: string } };
          ...
          <tbody className="divide-y divide-zinc-800/60">
            {sorted.map((r) => (
              <Fragment key={r.ticker}>
                <tr className="group hover:bg-[#151a2a]/80 transition-colors cursor-pointer" onClick={() => setExpanded(expanded === r.ticker ? null : r.ticker)} title={r.companyProfile ? r.companyProfile.name : r.ticker}>
                ...
                {expanded === r.ticker && r.companyProfile ? (
                  <tr className="bg-[#0B0E14]"><td colSpan={7} className="px-3 py-3 text-xs leading-relaxed text-zinc-400"><span className="font-semibold text-zinc-200">{r.companyProfile.name}</span> — {r.companyProfile.description} <a href={r.companyProfile.sourceUrl} target="_blank" rel="noopener noreferrer" className="ml-2 font-mono text-amber-300 hover:underline">idx.co.id ↗</a></td></tr>
                ) : null}
              </Fragment>
            ))}
